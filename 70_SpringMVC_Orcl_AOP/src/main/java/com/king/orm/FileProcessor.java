...
public class FileProcessor {
	...
	public void uploadFiles(MultipartFile[] files, int pno) {
		...
		for (MultipartFile file : files) {
			...
			logger.info(">>>>orgFileName :" + orgFileName);//orgFileName :sandbar-869195_1920.jpg
			logger.info(">>>>getName :" + file.getName()); //파라미터이름 >>>>getName :files
			...
			try {
				file.transferTo(storeFile); //이파일은 포문에 있는 파일임, storeFile에 파일 껍데기를 씌워줌
